...
export  const getIngredients=()=>async dispatch=>{
    const response = await axios.get('http://localhost:49652/api/ingredients');
    dispatch({
...
export const deleteIngredient =id=>async dispatch=>{
    await axios.delete(`http://localhost:49652/api/ingredient/delete/${id}`);
    dispatch({
...
export const showIngredient=id=>async dispatch=>{
    const response=await axios.get(`http://localhost:49652/api/ingredient/show/${id}`);
    dispatch({
...
export const addIngredient=dish=>async dispatch=>{
    const response = await axios.post('http://localhost:49652/api/ingredient/add/',dish);
    dispatch({
        type:ADD_INGREDIENT,
        payload:response.data
    })
